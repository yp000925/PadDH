...
from pad.utils import clear_color
...
class GaussianDiffusion:
    ...
    def p_sample_loop(self,
        ...
        for idx in pbar:
            ...
            # 在ps的算法里没有用到noisy_measurement 只有在projection 的算法里才会用到
            ...
            # check grad_map
            # img, distance,grad_map = measurement_cond_fn(x_t=out['sample'],
            #                           measurement=measurement,
            #                           noisy_measurement=noisy_measurement,
            #                           x_prev=img,
            #                           x_0_hat=out['pred_xstart'],
            ...
            self.distance.append(distance.item())
            if record:
                if idx % 10 == 0:
                    ...
                    # save x_0_hat
                    # x_0_hat = out['pred_xstart']
                    # file_path_x_0 = os.path.join(save_root, f"progress/x0_{str(idx).zfill(4)}.png")
                    # plt.imsave(file_path_x_0, clear_color(x_0_hat.detach_()))

                    # save grad_map, need to uncomment the code above for getting grad_map
                    # file_path_grad = os.path.join(save_root, f"progress/grad_{str(idx).zfill(4)}.png")
                    # plt.imsave(file_path_grad, clear_color(grad_map.detach_()))
...
@register_sampler(name='ddpm_gb')
class GB(SpacedDiffusion):
    '''
    DDPM with Gradient correction on x_0_hat
    '''
    def p_mean_variance(self, model, x, t, measurement_cond_fn, measurement):
        model_output = model(x, self._scale_timesteps(t))
        ...
        model_mean, pred_xstart = self.mean_processor.get_mean_and_xstart(x, t, model_output)
        corrected_xstart, distance = measurement_cond_fn(x_t = pred_xstart, measurement=measurement,
                                                x_prev = pred_xstart, x_0_hat=pred_xstart)
        model_mean_corrected = self.mean_processor.q_posterior_mean(corrected_xstart,x,t)
        model_variance, model_log_variance = self.var_processor.get_variance(model_var_values, t)

        assert model_mean.shape == model_log_variance.shape == pred_xstart.shape == x.shape == corrected_xstart.shape
        return {'mean': model_mean_corrected,
                'variance': model_variance,
                'log_variance': model_log_variance,
                'pred_xstart': corrected_xstart,
                'x_0_before_corrected': pred_xstart,
                'distance':distance}

    def p_sample(self, model, x, t, measurement, measurement_cond_fn):
        out = self.p_mean_variance(model, x, t, measurement=measurement,
        ...
        return {'sample': sample, 'pred_xstart': out['pred_xstart'],
                'x_0_before_corrected': out['x_0_before_corrected'],'distance':out['distance']}
    ...
    def p_sample_loop(self,
        ...
        for idx in pbar:
            time = torch.tensor([idx]*img.shape[0],device = device)
            img = img.requires_grad_()
            out = self.p_sample(model, img, time, measurement=measurement,
                                measurement_cond_fn=measurement_cond_fn)

            img = out['sample'].detach_()
            distance = out['distance']
            x_0_before_corrected = out['x_0_before_corrected'].detach_()
            pred_xstart = out['pred_xstart'].detach_()

            pbar.set_postfix({'distance':distance}, refresh=False)
            if record:
                if idx % 10 == 0:
                    file_path = os.path.join(save_root, f"progress/x_{str(idx).zfill(4)}.png")
                    plt.imsave(file_path, clear_color(img))
                    file_path = os.path.join(save_root, f"progress/x0_{str(idx).zfill(4)}_b.png")
                    plt.imsave(file_path, clear_color(x_0_before_corrected))
                    file_path = os.path.join(save_root, f"progress/x0_{str(idx).zfill(4)}_a.png")
                    plt.imsave(file_path, clear_color(pred_xstart))
